...
void test_queue_creation()
{
	...
	assert(str_q.len == 0);
	assert(str_q.a_len == 8);
	assert(int_q.len == 0);
	assert(int_q.a_len == 8);
}
...
void test_int_q_dispose()
{
	...
	assert(int_q.fst_s == NULL);
	assert(int_q.scd_s == NULL);
}
...
void test_str_q_dispose()
{
	...
	assert(str_q.fst_s == NULL);
	assert(str_q.scd_s == NULL);
	assert(str_q.len == 0);
	assert(str_q.a_len == 0);
}


void test_queue_students_enqueue()
{
...
void test_queue_students_dequeue()
{
	students_group aux;
